...
public class CombatMacro {
    ...
    public void onTick(Minecraft client) {
        ...
        // 1. DYNAMIC STAIR/SLAB AUTO-STEP FIX
        ...
        if ((client.options.keyUp.isDown() || client.options.keyLeft.isDown() || client.options.keyRight.isDown()) && moveDist < 0.005) {
            stuckTicks++;
            // Never jump if on stairs/slabs to prevent jittering
    ...
    private void executeAdaptiveAttack(Minecraft client) {
        ...
        boolean isBoss = timeSinceFirstHit > 5000; // 5 continuous seconds
        
        if (!isBoss) {
            if (currentTime >= nextClickTime) {
                injectHardwareClick(client);
                nextClickTime = currentTime + 300 + random.nextInt(60); // 3-4 CPS normal mode
            }
        } else {
            ...
            if (burstClicksRemaining > 0) {
                if (currentTime >= nextClickTime) {
                    ...
                    nextClickTime = currentTime + 50 + random.nextInt(30); // 5-8 CPS burst mode
                }
    ...
    private void smoothTravelLookTowards(Minecraft client, float targetYaw, float targetPitch) {
        humanizedLookTowards(client, targetYaw, targetPitch, 12.0f); // Smooth max speed 12.0 deg/tick
    }
    ...
    private void humanizedLookTowards(Minecraft client, float targetYaw, float targetPitch, float maxSpeedCap) {
        ...
        float maxYawSpeed = Math.min(maxSpeedCap, Math.max(3.0f, yawAbs * 0.35f));
        float maxPitchSpeed = Math.min(maxSpeedCap * 0.7f, Math.max(2.0f, pitchAbs * 0.35f));
    ...
    private boolean hasClearLineOfSight(Minecraft client, Vec3 start, Vec3 end) {
        ...
        for (int i = 1; i < steps; i++) {
            BlockPos pos = new BlockPos((int)cd ~/mobileminer
rm -f src/main/java/com/mobileminer/CombatMacro.java
cat << 'EOF' > src/main/java/com/mobileminer/CombatMacro.java
package com.mobileminer;
