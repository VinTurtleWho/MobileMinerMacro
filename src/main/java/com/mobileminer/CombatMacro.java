...
public class CombatMacro {
    ...
    // Lazy Aim Offsets (Changes per new target)
    ...
    public void toggle(Minecraft client) {
        if (!isCombatActive) {
            isCombatActive = true;
            sendMessage(client, "§c[MobileMiner] Combat Mode Activated. Human Strafing ON.");
        } else {
    ...
    public void onTick(Minecraft client) {
        ...
        // 3. Find or validate target (USING CLUSTER MATH)
        if (currentMob == null || !isTargetValid(currentMob, client)) {
            if (currentMob != null) stopMovement(client); 
            
            currentMob = scanForBestClusterMob(client, 30);
            currentPath = null;
            currentMobFirstHitTime = 0; 
            
            // Generate a random messy aim offset for this specific mob
            if (currentMob != null) {
                targetOffsetX = (random.nextDouble() - 0.5) * 0.4;
                targetOffsetY = (random.nextDouble() - 0.5) * 0.4;
        ...
        // ALWAYS Stare at the Mob (Decoupled Head)
        lookAtLazyTorso(client, actualTarget);

        // 4. COMBAT BRAIN
        if (distance <= 3.2) {
            stopMovement(client); // In range, stop strafing
        ...
        // 6. Look-Ahead Smoothing + WASD Strafing
        if (currentPath != null && !currentPath.isEmpty()) {
    ...
    private void applyWASDKeys(Minecraft client, float moveYaw) {
        ...
        // Determine which keys to press based on where our camera is facing vs where we need to walk
        if (diff > -60 && diff < 60) w = true; // Mostly forward
        if (diff < -120 || diff > 120) s = true; // Mostly backward
        if (diff > 30 && diff < 150) d = true; // Strafe Right
        if (diff < -30 && diff > -150) a = true; // Strafe Left
    ...
    private boolean canJump(Minecraft client) {
        ...
        return client.level.getBlockState(headPos).isAir(); // Only jump if the ceiling is clear!
    }
    ...
    private void lookAtLazyTorso(Minecraft client, Entity entity) {
        ...
        // 1. THE LAZY ZONE: If we are within 12 degrees, stop tweaking the mouse. Just let it rest.
        if (Math.abs(yawDiff) < 12.0f && Math.abs(pitchDiff) < 12.0f) {
            return; 
        }

        // 2. SPEED CLAMPING: Cap rotation to ~20 degrees per tick so it physically cannot snap its neck
        yawDiff = Math.max(-20.0f, Math.min(20.0f, yawDiff));
        pitchDiff = Math.max(-15.0f, Math.min(15.0f, pitchDiff));
    ...
    private Entity scanForBestClusterMob(Minecraft client, double range) {
        ...
        for (Entity target : validTargets) {
            ...
            // Count how many valid targets are in a 5-block radius of THIS target
            ...
            // Subtract 4 distance points for every friend. The bot prefers juicy groups!
    ...
    // COMPILER-PROOF LINE OF SIGHT RAYCAST
    private boolean hasClearLineOfSight(Minecraft client, Vec3 start, Vec3 end) {
        double dist = start.distanceTo(end);
        int steps = (int) Math.ceil(dist * 2); // Check every half block
    ...
    private boolean isTargetValid(Entity entity, Minecraft client) {
        ...
        if (unreachableEntityBlacklist.containsKey(id)) {
            // NEW FIX: Only shatter the lock if we have a direct, unobstructed visual line of sight to it!
            if (hasClearLineOfSight(client, client.player.getEyePosition(), entity.getEyePosition())) {
                unreachableEntityBlacklist.remove(id); // I see him! Lock broken.
            } else {
                return false; // He is still behind a wall in the crypts. Ignore.
            }
        }
