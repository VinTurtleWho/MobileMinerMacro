...
public class CombatMacro {
    ...
    // The ADHD Scanner (Dynamic Re-Targeting)
    private long lastTargetScanTime = 0;

    // Center-Mass Lazy Aim Offsets
    ...
    public void onTick(Minecraft client) {
        ...
        // 3. Dynamic Re-Targeting (The ADHD Brain - Checks every 500ms)
        if (currentMob == null || !isTargetValid(currentMob, client) || (currentTime - lastTargetScanTime > 500)) {
            ...
            if (currentMob != null && !isTargetValid(currentMob, client)) {
                stopMovement(client); // Drop dead targets instantly
            }
            
            Entity bestMob = scanForBestClusterMob(client, 30);
            
            // Only drop our current target if we found a genuinely better cluster
            if (bestMob != null && bestMob != currentMob) {
                currentMob = bestMob;
                currentPath = null;
                currentMobFirstHitTime = 0; 
                
                // Tighten the random offset to center-mass ONLY (-0.15 to +0.15 blocks)
        ...
        // 4. PATH-GAZING: Check if we can actually see the Ghoul
        ...
        if (canSeeTarget || (currentPath == null || currentPath.isEmpty())) {
            // We have eyes on the target (or we are in direct chase). Lock onto torso.
            lookAtLazyTorso(client, actualTarget);
        } else {
            // Target is behind a wall. Look at our walking path so we don't look like an X-Ray cheater.
            BlockPos nextWaypoint = currentPath.get(0);
            double targetX = nextWaypoint.getX() + 0.5;
            double targetZ = nextWaypoint.getZ() + 0.5;
            double targetY = nextWaypoint.getY() + 1.2; // Look slightly up from the floor
        ...
        // 5. COMBAT BRAIN
        if (distance <= 3.2 && canSeeTarget) { // Only swing if we can physically hit him!
            stopMovement(client); 
        ...
        // 7. Look-Ahead Smoothing + WASD Strafing
        if (currentPath != null && !currentPath.isEmpty()) {
            BlockPos nextWaypoint = currentPath.get(0);
    ...
    private void lookAtLazyTorso(Minecraft client, Entity entity) {
        ...
        // THE LAZY ZONE: If we are close to the center mass, rest the mouse completely.
    ...
    // EXPONENTIAL EASING CURVES (The Flick & Rest System)
    private void humanizedLookTowards(Minecraft client, float targetYaw, float targetPitch) {
        ...
        // Dynamic Rotational Speed limits
        float maxYawSpeed;
        if (yawAbs > 100) maxYawSpeed = 70.0f;       // Massive flick
        else if (yawAbs > 45) maxYawSpeed = 45.0f;   // Fast turn
        else if (yawAbs > 15) maxYawSpeed = 25.0f;   // Decelerating
        else maxYawSpeed = Math.max(3.0f, yawAbs * 0.4f); // Smooth glide into target
    ...
    private Entity scanForBestClusterMob(Minecraft client, double range) {
        ...
        for (Entity target : validTargets) {
            ...
            // Subtract points for cluster density to prioritize hordes
