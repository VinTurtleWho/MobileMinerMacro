...
public class CombatMacro {
    ...
    public void onTick(Minecraft client) {
        ...
        if ((client.options.keyUp.isDown() || client.options.keyLeft.isDown() || client.options.keyRight.isDown()) && moveDist < 0.005) {
            stuckTicks++;
            if (stuckTicks > 10 && canJump(client) && !onStairOrSlab) { 
            ...
            if (currentPath != null && !currentPath.isEmpty()) {
                if ((currentPath.get(0).getY() <= client.player.getY() || onStairOrSlab) && !client.player.horizontalCollision) {
        ...
        if (currentMob == null || !isTargetValid(currentMob, client) || (currentTime - lastTargetScanTime > 400)) {
            if (currentMob != null && !isTargetValid(currentMob, client)) stopMovement(client); 
        ...
        if (canSeeTarget || (currentPath == null || currentPath.isEmpty())) {
            ...
            smoothTravelLookTowards(client, pathYaw, pathPitch);
        }
        ...
        if (currentPath == null && !calculatingPath) {
            ...
            Pathfinder.calculatePathAsync(client, client.player.blockPosition(), mobPos).thenAccept(path -> {
                if (path == null || path.isEmpty()) pathFailed = true; 
    ...
    private boolean isStandingOnStairOrSlab(Minecraft client) {
        ...
        BlockState stateAtFeet = client.level.getBlockState(feetPos);
        BlockState stateBelow = client.level.getBlockState(belowPos);
        return stateAtFeet.getBlock() instanceof StairBlock || stateAtFeet.getBlock() instanceof SlabBlock ||
               stateBelow.getBlock() instanceof StairBlock || stateBelow.getBlock() instanceof SlabBlock;
    }
    ...
    private void navigateWithStrafing(Minecraft client, BlockPos waypoint, double dx, double dz) {
        ...
        if ((waypoint.getY() > client.player.getY() || client.player.horizontalCollision) && stuckTicks == 0 && canJump(client) && !isStandingOnStairOrSlab(client)) {
            client.options.keyJump.setDown(true);
        }
    }
    ...
    private void directStrafeChase(Minecraft client, Entity mob) {
        ...
        applyWASDKeys(client, moveYaw);
        client.options.keySprint.setDown(true);
        if (client.player.horizontalCollision && stuckTicks == 0 && canJump(client) && !isStandingOnStairOrSlab(client)) {
            client.options.keyJump.setDown(true);
        }
    }
    ...
    private void lookAtCenterMass(Minecraft client, Entity entity) {
        ...
        float targetPitch;
        if (dist < 0.3) targetPitch = client.player.getXRot(); 
        else {
        ...
        if (Math.abs(yawDiff) < 2.5f && Math.abs(pitchDiff) < 2.5f) return; 
        humanizedLookTowards(client, targetYaw, targetPitch, 45.0f);
    }

    private void smoothTravelLookTowards(Minecraft client, float targetYaw, float targetPitch) {
        humanizedLookTowards(client, targetYaw, targetPitch, 12.0f); 
    }

    private void humanizedLookTowards(Minecraft client, float targetYaw, float targetPitch, float maxSpeedCap) {
        float currentYaw = client.player.getYRot();
        float currentPitch = client.player.getXRot();
        ...
        float maxYawSpeed = Math.min(maxSpeedCap, Math.max(3.0f, Math.abs(yawDiff) * 0.35f));
        float maxPitchSpeed = Math.min(maxSpeedCap * 0.7f, Math.max(2.0f, Math.abs(pitchDiff) * 0.35f));
        ...
        float stepMult = f * f * f * 8.0f * 0.15f;

        int mouseDeltaX = Math.round((yawDiff * 0.45f) / stepMult);
        int mouseDeltaY = Math.round((pitchDiff * 0.45f) / stepMult);
    ...
    private Entity scanForBestClusterMob(Minecraft client, double range) {
        ...
        for (Entity entity : client.level.entitiesForRendering()) {
            ...
            if (isTargetValid(entity, client) && client.player.distanceTo(entity) < range) {
        ...
        for (Entity target : validTargets) {
            ...
            for (Entity friend : validTargets) {
                if (friend != target && target.distanceTo(friend) <= 5.0) friendsNearby++;
            }
    ...
    private boolean hasClearLineOfSight(Minecraft client, Vec3 start, Vec3 end) {
        ...
        for (int i = 1; i < steps; i++) {
            ...
            if (client.level.getBlockState(pos).isSolidRender()) return false; 
        }
        return true; 
    }
    ...
    private boolean isTargetValid(Entity entity, Minecraft client) {
        ...
        if (deadEntityBlacklist.containsKey(id)) {
            if (System.currentTimeMillis() - deadEntityBlacklist.get(id) > 3000) deadEntityBlacklist.remove(id);
            else return false; 
        }

        if (unreachableEntityBlacklist.containsKey(id)) {
            if (hasClearLineOfSight(client, client.player.getEyePosition(), entity.getEyePosition())) unreachableEntityBlacklist.remove(id); 
            else return false; 
        }
